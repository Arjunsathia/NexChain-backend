 ...
 */
const executeBuy = async (userId, coinData, quantity, price, totalCost, fees, deductBalance = true) => {
    ...
    let purchasedCoin = await PurchasedCoin.findOne({ user_id: userId, coin_id: coinId });
    ...
    return { success: true, purchasedCoin, newBalance: user ? user.virtualBalance : undefined };
};
...
const executeSell = async (userId, coinId, quantity, price) => {
    ...
    const purchases = await PurchasedCoin.find({ user_id: userId, coin_id: coinId }).sort({ purchaseDate: 1 });
    ...
    if (totalAvailable < quantity - 0.00000001) {
            throw new Error(`Insufficient coin quantity. Owned: ${totalAvailable}, Trying to sell: ${quantity}`);
    }
